...
/**
 * @file page.js (rota '/info/cas-token/whitepaper')
 * @description Renderiza o whitepaper do CAS Token a partir do markdown
 *              em /public/tokens/cas-whitepaper.md, com formatação estilo
 *              tutoriais (ReactMarkdown + remark-gfm).
 */
...
export default function WhitepaperPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/tokens/cas-whitepaper.md');
        if (!res.ok) throw new Error('Falha ao carregar whitepaper');
        const text = await res.text();
        setContent(text);
      } catch (err) {
        console.error('[WhitepaperPage] load error:', err.message);
      ...
      <div className="space-y-6">
        ...
        <div className="card border-red-500/40 bg-red-500/10 text-red-300">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold">Erro ao carregar whitepaper</p>
    ...
    <div className="space-y-6">
      <header className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-white">CAS Whitepaper</h1>
        </div>
      </header>
