...
export const CERTIFICATE_ABI = [
  ...
  'function casDeposits(address recipient, uint256 phaseId) view returns (uint256)',
...
export function buildCertificateManifest({ config, phase, certificate, recipientName, txHash = '' }) {
  ...
  return {
    ...
    certificate: {
      ...
      certificateType: phase.certificateType || phase.name || '',
      skillsDescription: phase.skillsDescription || '',
      achievementSummary: phase.achievementSummary || '',
      courseHours: phase.courseHours || 40,
