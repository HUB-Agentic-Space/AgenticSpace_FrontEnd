---
description: When the user is acting as an ERC-8183 seller on the single selected-protocol AgentCore runtime — implementing the value notify_funded produces, tuning the deterministic rule-based quote price, understanding A2A async delivery vs MCP synchronous delivery, handling submitted jobs, defending against buyer disputes, and ensuring LLM credit continuity during long jobs.
---

> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/selling-via-8183.md`. Route here via the router's decision tree.
...
Audience: Windsurf in a working repo with a funded wallet (tBNB + U) and an
Agent that produces some valuable output (text, classification, image — whatever).

**Different from**:
- `scaffolding-agent.md` (same directory) — creates the project (this playbook runs after)
- `adding-to-project.md` (in this same directory) — wires the seller runtime into an existing project (the static setup, not the runtime behavior)
- `operating.md` (same directory) — generic ops (dev / doctor / balance); jumps here for seller-specific decisions

This playbook owns: **implement the `notify_funded` value → deploy the agent → defend disputes**.
...
- The agent sub-project is emitted (`<workspace>/app/agent/main.py` for A2A or `mcp_main.py` for MCP, plus `signing.py`). If not, run `bag init` or read the `adding-to-project.md` playbook first.
...
**Hard rules**:
- The endpoint must be **reachable** when registered. For A2A, smoke test the normalized card URL directly. For MCP, connect an MCP client to the deployed `/mcp` URL. Chain doesn't verify reachability, but buyers will see failures.
...
| Agent unreachable (buyer reports) | AgentCore runtime is down, DNS wrong, or the OAuth2 bearer is missing/expired | `bag deploy status` (or `agentcore status` — read the `use-aws-agentcore.md` playbook) to inspect; redeploy via `bag deploy agent` |
...
- the `wiring-llm-tools.md` playbook (in this same directory) — wiring read-only chain tools into the Agent's LLM
