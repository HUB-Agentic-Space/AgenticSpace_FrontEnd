...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/buying-from-bazaar.md`. Route here via the router's decision tree.
...
| `X402PolicyError` / `PolicyViolation` | Signing allowlist — see `extending-signing.md` (same directory) |

**Different from**:
- `buying-via-8183.md` (same directory) — buying from an
  **ERC-8183 seller agent** (jobs, disputes, settle windows). This file is
  about flat pay-per-call x402 APIs; no job lifecycle.
- `extending-signing.md` (same directory) — the signing-policy allowlist
