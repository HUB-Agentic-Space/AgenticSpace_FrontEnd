...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/operating.md`. Route here via the router's decision tree.

# bnbagent-studio-operating

Procedure for **day-to-day operation** of an existing bnbagent-studio project.
Audience: Windsurf in a v0.0.1 workspace (thin root with the `agentcore/`
...
**Different from**:
- `scaffolding-agent.md` (in this same directory) — creates a brand new project
- `selling-via-8183.md` (in this same directory) — seller flow (`run_work` value, rule-based quote pricing, dispute defense)

**Deep-dive playbooks** — plain markdown files in this SAME directory.
READ the file when the topic comes up — don't answer from memory:
- `use-aws-agentcore.md` — the native `agentcore` CLI (dev / deploy / status / invoke / logs / traces / fetch / validate / stop) + AWS prerequisites
- `using-twak-wallet.md` — `[wallet].kind = "twak"` create / fund / SIWE-bind / container deploy / limitations
- `extending-signing.md` — `PolicyViolation` / `X402PolicyError` diagnosis + extending the EIP-712 allowlist
- `adding-to-project.md` — adding the seller runtime to an existing Python project
- `buying-via-8183.md` — buyer flow (find provider → buy → fetch → settle)
...
`selling-via-8183.md` (same directory); for the buyer side, read the buying playbook.
...
| "settle job X" | `bag erc8183 settle <id> --action approve\|reject\|dispute` (default `approve`) — **seller's manual step** after the dispute window; deeper context in `selling-via-8183.md` (same directory) |
| "submit work for job X" | **seller action** — read `selling-via-8183.md` (same directory) for the submit/dispute flow |
...
into Windsurf) provide `balance_u` / `balance_native` as well — but the CLI
...
`buying-via-8183.md` playbook (see the Deep-dive playbooks
list above). From the seller side, defending a dispute is
covered in `selling-via-8183.md` (same directory).
