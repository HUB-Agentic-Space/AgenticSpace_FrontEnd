...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/scaffolding-agent.md`. Route here via the router's decision tree.

# bnbagent-studio-scaffolding-agent

Procedure for **greenfield** seller creation. Audience: Windsurf (or another
agent) running in an empty directory with shell + edit access.

**Different from** the `adding-to-project.md` playbook (in this same directory): that one adds to an existing
...
> new`, zero-deposit-activates Pieverse, and prints faucet URLs). **You (the
> agent) drive `bag init` non-interactively**, so that auto-flow does NOT fire —
   ...
   - **twak** (fully supported — opt in with `--wallet-kind twak`): `bag init` writes `[wallet].twak_home =
     ...
     **Tell the user exactly what to pick at each wizard step** — it is not
     ...
     - **Step 2 (wire up harnesses): SELECT NONE, press ENTER** (don't press
       SPACE/`a`) — never wire twak's signing MCP into Windsurf / Cursor /
     ...
     `using-twak-wallet.md` playbook (in this same directory).
     ...
     the `using-twak-wallet.md` playbook (in this same directory).
...
7. **Recipe code is already emitted by `bag init`** — the `app/agent/`
   ...
   `app/agent/mcp_main.py` (MCP) (see `selling-via-8183.md` in this same directory).
...
- **`bag init` runs wallet onboarding only on a human TTY** (evm-local: prompts
  ...
  argv). When **the agent** runs `bag init` (non-interactively, via the shell
