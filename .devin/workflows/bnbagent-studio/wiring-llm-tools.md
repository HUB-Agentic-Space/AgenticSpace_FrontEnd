...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/wiring-llm-tools.md`. Route here via the router's decision tree.
...
sub-project as `app/agent/tools.py`); for other frameworks you write
...
Windsurf (or another agent) editing the user's workspace. The user has run
...
- The user is doing dev-time debugging via Windsurf — that's the `bag` CLI
...
Read tools (this playbook) are safe-ish — worst case the LLM gives wrong info.
...
(FastMCP `/mcp` for buyers). This playbook wires read-only chain queries into the
