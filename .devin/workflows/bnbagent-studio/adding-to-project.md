...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/adding-to-project.md`. Route here via the router's decision tree.

# bnbagent-studio-adding-to-project

Procedure for adding `bnbagent-studio`'s **single selected-protocol seller** to an
**existing** Python repo. Audience: Windsurf (or another agent) running with
...
`wiring-llm-tools.md`). ALL signing stays in `app/agent/signing.py` —
...
synchronously inside the tool call (see `selling-via-8183.md` (same directory) and
...
- `selling-via-8183.md` (same directory) — the runtime seller flow (negotiate/notify_funded/settle, disputes).
