...
> **Playbook** of the `bnbagent-studio` workflow — at `.devin/workflows/bnbagent-studio/extending-signing.md`. Route here via the router's decision tree.

# bnbagent-studio-extending-signing

Procedure for **extending the EIP-712 signing allowlist** in a bnbagent-studio project.
Audience: Windsurf helping a user who hit `PolicyViolation` or wants to integrate a non-default x402 service / custom contract.
...
**Different from**:
- `operating.md` (same directory) — general ops & doctor / balance / job status
- `adding-to-project.md` (same directory) — adds capabilities (`bnbagent-sdk` modules)

This playbook is specifically the **signing-permission** axis. Funding is "do I have the money"; signing policy is "is the wallet allowed to sign this".
